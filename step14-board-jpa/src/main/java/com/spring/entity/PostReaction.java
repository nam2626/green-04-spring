...
/**
 * [게시글 반응(좋아요/싫어요) 엔티티 클래스]
 * 
 * @Entity: JPA가 관리하는 데이터베이스 매핑 객체임을 선언합니다.
 * @Table: 매핑할 테이블 정보 및 제약조건을 설정합니다.
 *         - name: 테이블 이름을 'post_reaction'으로 지정합니다.
 *         - uniqueConstraints: 한 명의 회원이 하나의 게시글에 단 하나의 반응만 남기도록 (member_id, post_id) 복합 유니크 제약을 겁니다.
 ...
 */
@Entity
@Table(name = "post_reaction", uniqueConstraints = @UniqueConstraint(columnNames = { "member_id", "post_id" }))
@Data
@NoArgsConstructor
@AllArgsConstructor
public class PostReaction {
  ...
  // @ManyToOne: N:1(다대일) 연관관계 매핑입니다. 하나의 회원(Member)은 여러 게시글 반응을 작성할 수 있습니다.
  ...
  // @ManyToOne: 하나의 게시글(Post)은 여러 개의 게시글 반응을 가질 수 있습니다. 지연 로딩을 적용합니다.
