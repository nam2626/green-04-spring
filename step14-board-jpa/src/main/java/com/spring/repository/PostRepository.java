 ...
 * [게시판 JPA 리포지토리 인터페이스]
 * 
 * JpaRepository<Post, Long>를 상속받음으로써 기본적인 CRUD 메서드(save, findById, findAll,
 * delete 등)가
...
public interface PostRepository extends JpaRepository<Post, Long> {
   ...
   *         - join fetch p.member:
   *         게시글(Post)을 조회할 때 연관된 회원(Member) 테이블도 함께 즉시 조인(Join)하여 SQL 한 번에 한꺼번에
   *         읽어옵니다.
